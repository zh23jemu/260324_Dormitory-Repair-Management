...
const routePathMap = {
  orders: '/orders',
  repairerOrders: '/repairer/orders',
  repairerStats: '/repairer/stats',
  dorms: '/dorms',
...
const activeTab = computed({
  get() {
    return typeof route.name === 'string' ? route.name : 'orders'
  },
  set(value) {
    const target = routePathMap[value] || '/orders'
...
function normalizeTabByRole() {
  const current = typeof route.name === 'string' ? route.name : ''
  const allowed = []
  if (isDorm.value) allowed.push('orders', 'dorms', 'announcements')
  if (isRepairer.value) allowed.push('repairerOrders', 'repairerStats')
  if (isAdmin.value) allowed.push('admin', 'config', 'orders', 'dorms', 'announcements')
  if (!allowed.length) return
  if (!allowed.includes(current)) {
    const fallback = isRepairer.value ? 'repairerOrders' : 'orders'
    router.replace(routePathMap[fallback])
  }
}
