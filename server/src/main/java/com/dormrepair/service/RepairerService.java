...
public class RepairerService {
    ...
    public Map<String, Object> statistics() {
        SecurityUtils.requireRole("repairer");
        JwtUser user = SecurityUtils.currentUser();
        Integer total = jdbcTemplate.queryForObject("select count(*) from repair_order where assigned_repairer_id = ?", Integer.class, user.id());
        Integer completed = jdbcTemplate.queryForObject("select count(*) from repair_order where assigned_repairer_id = ? and status = 'completed'", Integer.class, user.id());
        Integer rated = jdbcTemplate.queryForObject("select count(*) from repair_order where assigned_repairer_id = ? and status in ('pending_rating', 'completed')", Integer.class, user.id());
        ...
        map.put("avgHandleHours", rated == null ? 0 : rated);
        return map;
    }
}
